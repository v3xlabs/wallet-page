    ...
    <div className="flex items-center gap-2">
      ...
      {quoting
        ...
        : (
            ...
            >
              {quote ? formatAmount(quote.receiveAmount) : (unavailable ? "—" : "0")}
            </span>
          )}
    </div>
    <div className="flex items-center justify-between gap-2">
      ...
      <span className="text-xs text-muted tabular-nums">
        {quoting || unavailable || token.priceUsd === 0 ? "" : formatUsd(quote?.receiveUsd ?? 0)}
      </span>
    </div>
  </Panel>
);
...
export const FlipButton = ({ flips, onFlip }: { flips: number; onFlip: () => void; }) => (
  <button
    ...
    className="relative z-10 mx-auto -my-4 flex size-9 cursor-pointer items-center justify-center rounded-full border border-primary bg-surface text-secondary shadow-sm transition-transform duration-300 hover:text-primary"
